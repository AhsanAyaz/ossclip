...
> **For agentic workers:** REQUIREMENTS, not designs. Every diagnosis below was read out of the source on 2026-07-31 and is stated with its file path; the approaches are deliberately left open. **Read §119.0 before starting anything here** — the ordering is the most load-bearing decision in this document, and three of these items are cheap only if done in the stated order.

**Status:** captured 2026-07-31 from a roadmap discussion. Nothing started. Findings numbering continues at **§119**.

**Numbering note (2026-07-31):** these items were originally labelled §118–§122, reserving findings numbers ahead of the work. R25 then landed the scene-count fix as **§118** — the number this document had reserved for multi-source input — so the items here are renumbered **§119–§123**. The lesson stands in both docs now: a plan reserves no numbers; a finding takes the next free one when it lands.

**Prerequisite:** [`2026-08-05-scene-count-from-content-structure.md`](./2026-08-05-scene-count-from-content-structure.md) — **shipped 2026-07-31 as R25 §118** (see `docs/PHASE1-FINDINGS.md`). See §119.0 for why it came first and what its landing means for the appetite here.

---

## 119.0 The ordering, and why it is the point
...
1. **§118** — the scene-count target. Shipped (R25); the prerequisite is met.
2. **§119** — multi-source input (concat-first).
3. **§120** — retake/blooper removal as a cut reason.
4. **§121** — user cuts, server-side first, then the editor.
5. **§122** — `Scene[]` authored by an agent, as a CLI step.
6. **§123** — freeform TSX: a decision to record, not a task to schedule.
...
## 119. Multi-source input — join the takes before anything measures them
...
## 120. Retake and blooper removal — the slot is already cut
...
## 121. User cuts — server-side first, editor second
...
### 121.1 Why the pipeline is ready for this
...
## 122. Scene authoring by an agent — as a CLI step, not an editor endpoint
...
### 122.1 The seam that already exists
...
## 123. Freeform TSX — a decision, recorded
...
### 123.1 Seven systems are keyed to the closed component enum
...
### 123.2 The threat model changes category
...
| risk | mitigation |
| --- | --- |
| Doing 119–122 concurrently rebuilds the resisted "agent-native studio shell" by accident | one at a time, each landing on real footage before the next starts |
| §120 silently ends the deterministic-cut guarantee | state it where the guarantee is currently claimed, or keep the detector deterministic |
| §121 drifts `splits` and pinned `timing` with no error | extend `reclampPinnedTiming`; `splits` needs a guard that does not exist yet |
| §119 regresses audio on multi-mic material | per-region thresholds, or document the limitation |
| Authoring gets built to solve a density problem §118 already solved | §118 has landed — re-ask on real footage whether 122 is still wanted before building it |
