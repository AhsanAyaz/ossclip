...
> **Status 2026-07-28:** Tasks A and B are done and verified on the author's own clip. Task C's detection, plumbing and render-time crop are done; the per-segment face measurement, producer awareness and mixed fixture (C5, C7, C8) are NOT.
>
> **Status 2026-07-29 — Task C's render-time cover REPLACED by bake-time normalization (option (a), decided with the author).** The author's verdict on the C-as-shipped render was that the product got worse: cover-filling a landscape strip into a 9:16 slot is a ~3.17× crop, so every framing boundary was a hard mid-scene zoom jump — nine per minute on the motivating clip, none aligned with a cut. Reproduced and measured on a synthetic alternating fixture before changing anything.
>
> What replaced it:
> - **Normalization (`packages/core/src/normalize.ts`).** A mixed source is baked to ONE field of view — the tightest the take ever shows (the strip; those stretches have nothing more to give). Every other segment is cropped to a window of the same shape, placed so the face sits at the same relative position as it does in the strip, then all segments are trim/crop/scale/concat-ed into a single uniform file carrying the mezzanine's encode settings. The whole downstream pipeline (face bias, source-text, cover, layouts, zoom, editor preview) runs its ordinary uniform-source path against the baked file. Verified end to end: the same scene straddling a framing boundary renders at identical scale on both sides.
> - **C5, closed.** `measureFaceInWindows` measures each segment's face inside its OWN rect (a single median across mixed framings averages two coordinate systems and points the crop at neither — it put the eyes at the top of the frame on the real clip). Its output places the bake windows; the stage's face comes from the normal `measureFace` on the baked file. `face.json`/`source-text.json` caches carry a `cacheTag` so a measurement of one file is never served for another.
> - **Boundary refinement.** Baking makes ±0.25s boundaries unacceptable (every frame on the wrong side gets the wrong window), so `detectContentRect` now re-scans ~1.6s around each coarse boundary at native fps and moves it to the midpoint of the two frames that actually disagree (`pickTransition`; cache v3). Measured on the fixture: boundaries land exactly on the change.
> - **Quality gate + option (b) fallback.** Normalizing is refused when covering the output from the strip would upscale past ×2.6 (`MAX_NORMALIZE_UPSCALE`; the author's clip sits at ×2.38). Refusal falls back to render-time FIT — the strip inset whole at natural size (`contentFitBox`, `contentCropMode: "fit"`), an honest inset rather than a fake full-frame shot — with a loud console warning that framing will visibly change. The cover-mode render path C6 built remains only as this fallback's machinery.
> - **C8, closed.** `make-fixture.mjs` emits `mixed-framing.mp4` at the real clip's exact geometry (1440×2560, 808px strip) — the dimensions matter, because the gate is measured in output pixels and a smaller strip would silently verify only the fallback.
> - **C7 (producer awareness), now moot for the normal path:** after normalization the producer sees a uniform source. It would matter again only for gated (fit-fallback) sources; deliberately not built until one exists.
>
> - **Task A** — `rejectCtaKeyword` drops referential filler before anything reads it. Verified: the run now logs `⚠ CTA keyword dropped — "number" names the reply rather than being it`, and the ChatMock scene renders the exchange the beat sheet planned ("Which one did you not know?" / "3") instead of a lone `"NUMBER"` pill. The caption is unstyled. **One plan step was wrong and was not followed:** A1/A2 assumed `chatBubbles`' single-bubble collapse was a bug. It is deliberate (FINDINGS §28b, `fit.ts:277`); the stale half is `applyCtaKeyword`, which is referenced only by its own test. Collapsing behaviour was left alone and the fix moved entirely to not setting the keyword. `applyCtaKeyword` is still dead code and still contradicts §28b — worth resolving, deliberately out of scope here.
> - **Task B** — one push per cut-free clip, then hold. Verified: `▸ zoom: 1 clip(s), 8s push then hold (2 segments)`, replacing `acoustic (24 phrase boundaries, 25 segments)`. Surfaced and fixed a real latent bug: `zoomScaleAt` matched segments CLOSED at both ends, so at a cut the previous clip's held scale won and the reset landed a frame late. B4 checked out — `stage.ts`'s head-margin budget was always computed at `ZOOM_MAX_SCALE`, so holding at peak needs no change.
> - **Task C** — C1/C2/C3 (timeline detection with anomaly absorption, denser 2 Hz sampling, `CACHE_VERSION` 2), C4 (**decided: never bake a mixed source**; a letterboxed stretch holds a LANDSCAPE picture that no ffmpeg crop turns into a portrait frame, so it is cover-cropped at render time where the face bias already lives), and C6 (`content-crop.ts` + `ContentCrop` in `VideoStage`) are implemented, with `content-rect.ts` split so the pure geometry can enter the Remotion bundle. **C5, C7 and C8 remain** — see "What is not done" at the end.
