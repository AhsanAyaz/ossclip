...
> **Status 2026-07-27 (remote session): all eight tasks implemented; e2e-verified in a real Chromium; hand-feel on real footage still owed by the author.**
>
> - **Task 7** shipped first as planned: `content-rect.ts` (cropdetect parsed like silencedetect, union across samples, per-side snap, implausibly-small refusal), cached as `content-rect.json`, consumed by face measurement, source-text scan, cover picker, `sourceAspect`, and the mezzanine — which stops being optional for a letterboxed source, since the bars are pixels in the file. Verified end to end on a letterboxed fixture (`make-fixture.mjs` now emits it): `video-top` renders solid picture where it was mostly bar.
> - **Task 8**: Task 7 rebuilt both preconditions (detect inside the content rect; stop squeezing frames into a fixed 9:16 box — the cascade is trained on undistorted faces). On top: a tilt sweep (±20°/±40° frame rotation, detections mapped back, strictly a fallback), shared by `measureFace` and the cover detector. Stated plainly in the code: this recovers TILTED heads, not TURNED ones — a true profile is out-of-plane and needs a profile cascade if the loud-miss log still fires. **The measured re-test on the motivating clip needs the author's footage**; the console now prints `N recovered by tilt sweep` so that run will quantify it.
> - **Task 1**: two multiplicative errors, both found by the new ±5% on-screen e2e assertion (the old stored-value assertion passed while the bug shipped — exactly the gap the plan named). (1) `editStyle` rendered inside SceneLayer's `scale(fitScale)` wrapper → fixed structurally: stored dx/dy stay composition-px truth and SceneLayer counter-divides at the one boundary that knows the scale (`compensateEdits`). (2) the page→composition factor came from the stage div's rect, but the Player letterboxes its canvas inside that box — fixed by asking the Player itself (`getScale()`). Known residual, on purpose: dragging while a scene's ENTRANCE spring is mid-flight previews slightly short (the stored value is the settled-frame truth); with Task 4 landing clicks mid-scene, that window is rarely where edits happen.
> - **Task 2**: selective swallow via a window CAPTURE-phase `pointerdown` listener (the Player toggles on pointerdown, and capture is the only point upstream of React's root). `stopPropagation` only, never `preventDefault` — cancelling pointerdown suppresses the compatibility mouse events the selection/drag logic runs on. Element presses and selection-box presses are the editor's; background presses reach the Player untouched. Both halves asserted in one e2e.
> - **Tasks 3+4+6** landed as one Timeline pass, as the plan suggested: one `timeAtX` mapping for every seek gesture; press-and-drag scrubbing with a grabbable playhead (grabbing the needle does NOT jump-seek); block clicks seek to the CLICKED time; block body-drag moves the scene via `moveTiming` (duration-preserving clamp, distinct from the edge-resize clamp) behind a 4px threshold, so a wobbling click still seeks and never writes a `timing` override. A body move pins exactly like an edge drag.
> - **Task 5**: SPACE toggles via `playerRef.toggle()`, guarded for inline edits, inspector fields/contentEditable, and focus on the Player's own play button (which already handles Space natively — firing twice would net to nothing).
> - **E2E infra facts worth keeping**: the container's headless Chromium ships NO H.264 decoder, so playback assertions read the Player's transport state (mirrored as `data-playing` on the stage) rather than `<video>.paused`; raw `page.mouse` never auto-scrolls, so the viewport is 1280×1000 to keep the timeline above the fold; `OSSCLIP_E2E_CHROMIUM` points the runner at a pre-installed browser; `--autoplay-policy=no-user-gesture-required` lets the keyboard-initiated play run.
