...
## 116. Above 45 seconds, nothing stops the producer under-planning
...
- [ ] **116a. A floor that applies above 45 seconds.** Whatever form it takes, a 64-second take with five enumerated points must not render three graphics. Derive the target from the content's structure where a structural signal exists, and fall back to runtime where it does not.
- [ ] **116b. Report the decision.** `report.txt` should say how many graphics were expected and how many survived, the same way the cut report justifies every cut. A count that silently under-delivers is exactly the failure this finding is about; it should not be able to happen quietly twice.

### 116.2 Why the obvious fix does not work — read before designing
...
### 116.3 The constraint this must not break
...
### 116.4 The caption collision this session also surfaced — diagnosed and fixed as §115

The 5-scene render showed two captions stacked at 46.00s. This plan originally guessed the mechanism was the open caption-band item in the Phase 1 header (anchors hand-tuned per layout rather than derived from live occupancy). **That guess was wrong**, and the real cause is worth knowing here because it interacts with scene count: adjacent `<Sequence>` windows were computed from a rounded *duration* instead of a rounded *end time*, so 8 of 76 caption transitions overran by exactly one frame. See §115.

The tie to this plan is the visibility, not the cause. All 8 collisions existed in every render; only the 3 that straddled a scene-cue boundary were *visible*, because only there do the two lines resolve to different anchors and stack at different heights. More scenes means more boundaries means more of the same latent defect showing up on screen — which is why the 3-scene renders looked clean and the 5-scene one did not. Raising the floor per 116a would have made a pre-existing bug look like a regression introduced by the floor.

Worth carrying forward as a habit: when a change increases scene count, latent per-boundary defects get more exposed, not more numerous.
