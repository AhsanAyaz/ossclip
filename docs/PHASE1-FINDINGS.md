...
**The failure path nobody had measured (2026-08-22).** Everything above was verified against agy; the FAILURE path was not, and it was wrong in both directions. The incident: a `--produce --aspect 16:9` run on an 11-minute take timed out twice at `--print-timeout 10m`, burned 25 minutes, and died with "Is Antigravity installed and logged in?" — on an agy that was installed, logged in and working. The hint was appended to every failure unconditionally, so the message sent the user to debug auth after a 25-minute wait. Measuring the two surfaces (agy 1.1.18, forced by running with `--print-timeout 2s` and with a junk `--model`) explains why nothing above caught it:

| failure | exit | stdout | stderr |
| --- | --- | --- | --- |
| print timeout fires | 1 | `{"status":"ERROR","error":"timeout waiting for response",…}` | *empty* |
| unknown model slug | 1 | `{"status":"ERROR","error":"invalid model selection (--model \"…\") …"}` | *empty* |
| bad flag / usage error | 2 | *empty* | usage text |

So agy speaks its OPERATIONAL failures through the stdout envelope and its PROCESS-level ones through stderr — and `run()` rejects a non-zero exit keeping only the stderr tail, discarding stdout. Two consequences, both live for the life of the provider. First, the message this paragraph's own fail-fast claim rests on ("run() embeds the stderr tail, so auth/bad-slug are matchable here") is false: on a real bad slug the rejection carried our echoed argv and nothing else, so `isNonRetryableAgyFailure` never matched, and the documented one-call fail-fast has never once fired in the field — every bad slug paid the second ~24k-token call it was written to avoid. Second, the suite could not see any of this, because `stubAgyFailing` wrote the reason to *stderr* — a shape agy does not produce. A stub that cannot fail the way the real tool fails is a green suite over an untested path.

The fix reads both surfaces: `run(…, { allowNonZero: true })`, `agyErrorText()` preferring the envelope's `error` and falling back to stderr, and `envelope.status === "SUCCESS"` as the only success test (an exit code we no longer see cannot be one). `classifyAgyFailure()` then names the failure — `auth | model | timeout | schema | unknown` — with the *timeout* and *model* anchors taken from the table above and *auth* left inferred, since establishing it would mean signing the user out. Guidance is gated on the class (the sign-in hint only for `auth`; a timeout names `--llm claude-cli`, `--llm gemini`, or dropping `--produce`), but the ATTEMPT FACTS print for every class — "2 attempts, 10m0s and 10m0s, --print-timeout 10m" — because they self-diagnose the hang regardless of the classification and stay true when the guess is wrong. Retry policy and `AGY_PRINT_TIMEOUT` are untouched; an adaptive timeout is a separate decision.

Claude Code's CLI had the same unconditional hint and a *different* surface, measured the same day: a bad `--model` slug exits 1 with `[claude-code:unrecognized_model] {…}` on stderr and a human sentence in the stdout envelope's `result`. Because it uses stderr — and because the prompt rides stdin there, so the argv echo is short — its rejection already carries the reason and needs no `allowNonZero`; only the hint gating and the attempt facts were applied. The `result` sentence is still dropped by the reject path, which is a smaller version of the same blindness and is left standing on purpose: no incident has asked for it yet.

Method note, since this is the second §132 claim to come from reasoning rather than measurement (the first was the provider survey itself): both corrections cost one command each — `agy --print-timeout 2s`, `claude --model nope`. Failure paths are cheap to measure and are exactly where an unmeasured assumption survives longest, because nothing exercises them until a user is already having a bad day.
...
**The fix is to key on something a cut cannot move.** Caption edits are keyed `w<ms>` by the word's SOURCE time (`captionKeyFor`), carried on `CaptionWord.srcStart` — the one deliberately source-timed field in a structure that is otherwise entirely output time. Split halves take an id minted once when the split is created (`mintSplitId`) and NEVER recomputed; `at` still moves under a re-cut, `id` does not. Decoupling the two made an id collision reachable that had been structurally impossible (`SPLIT_MIN_PIECE_SEC` used to forbid two splits within 0.5ms, so time-derived ids could not clash) — a split minted at 1.2s and re-anchored to 0.6s still holds `"1200"`, and ⌘B at 1.2s again asks for an id that is taken — so the mint disambiguates with a counter rather than a nonce, because the value is persisted in the user's file and has to be reproducible from the doc alone. The legacy derivation is load-bearing in the opposite direction: a pre-§137 split upgrades to an id computed from the output milliseconds of the `at` its file holds, so an existing `scene-0@600` keeps matching the half it always named — **for any doc that has not already been through a re-anchoring produce run**. That qualifier is not decoration, and the first draft of this section omitted it (final whole-branch review, Important 3). `at` is precisely the field a re-cut moves, so on a doc the bug already damaged the derivation reproduces the CURRENT time and not the original: the field workdir's live `overrides.json` holds `splits: [0]`, mints `"0"`, and its saved `scene-0@600` matches nothing. **A doc damaged before this fix landed cannot recover its split-half overrides from itself.** Only `overrides.json.bak` still holds the pre-cut `splits: [0.6]`, which is why the write gate below must not spend it, and why the plan's own manual-verification script only appears to prove the guarantee — its step 3 restores the `.bak` first. There is no better mechanism available: the original ms is genuinely gone from a re-anchored file and nothing on disk records it. A "cleaner" mint for legacy splits would have discarded saved work on every project on disk as well as this one.
...
**Nothing is dropped silently, and the invisibility is what let this reach a rendered video.** `migrateCaptionKeys` reports every edit it would not commit through `unresolved`, with a reason (`not-found`, `ambiguous`, `unanchorable`, `collision`, `superseded`) because each one asks something different of the user — blaming the cut for all of them sends somebody hunting for a word still sitting on screen. `applyCaptionEdits` reports through `dropped`. Both channels existed in some form before; the editor threw `applyCaptionEdits(...).dropped` away, which made the function's own "reported, never silently discarded" comment false at its only real call site, and that is the whole reason four vanished retypes looked like nothing had happened. The repair also has to REACH DISK, and the editor alone cannot get it there: `migrateLoadedDoc` runs on the load paths (`App.tsx`'s `loadProduction`, and the reload after a completed render), then hands the result to `edits.load`, which sets `dirty: false` — and the editor only saves when dirty. So the migration reaches the screen and not the file, and a user could open the old project, see every retype back where they left it, click Render, and ship a video with none of them: a state strictly more convincing than the truth. (Not the same defect as the discarded `dropped` above, which lived in the APPLY memo — the two are easy to conflate and are in different files.) "Never written" is the wrong verb for it, though, and the first draft used it: the editor's Save PUTs whatever it holds the moment the user changes anything else, so the migrated doc absolutely can reach disk — it just does so as a side effect of an unrelated edit, which is worse than either alternative, and for one round it did so having stripped the edits the migration could not place. So `produce` migrates too, through the one sanctioned `overrides.json` write with its `.bak` and atomic rename. A legacy key's resolvability DECAYS — it is found by the word it names, so the next re-plan that rewrites that word retires it for good — which makes the produce-side write the only durable repair in the product.

**A repair the run cannot do is not a licence to delete, and this is where the fix nearly re-created the bug.** Both write-backs first kept `migrateCaptionKeys`' `edits` and nothing else, which quietly makes every unplaceable edit a DELETE — on a run the user only asked to render. `captionEditsToKeep` is the rule both callers now share: everything placed, plus every unresolved edit under its original key, retiring only `superseded` (a newer source-keyed edit already covers that word, so keeping the legacy duplicate would re-report the same collision forever). An unresolved key costs nothing to leave: it addresses no word, so it applies to nothing, and it is named in the log every run — while deleting it forecloses the next run placing it against a different cut. The apply pass is therefore handed a NARROWER set than the doc, in both apps (`migration.edits` in produce, `sourceKeyedCaptionEdits` in the editor); handing it the whole doc would report each preserved positional key as "no word sits at that moment any more", which is the misdiagnosis this section already removed once. The write GATE moved for the same reason: it is `reanchoredKeyCount(...) > 0`, work actually done, not "the migration had something to say". Gated on the latter, the first `ossclip produce` after upgrading the field workdir wrote on a run that repaired nothing — and because a caption migration is independent of the cut, it wrote where the pre-§137 gate wrote nothing at all, spending `overrides.json.bak` (that user's only pre-cut save, and per the split-id paragraph above the sole route back to their deleted half) on a copy of the already-damaged document. Repairing the captions would have destroyed the evidence needed to repair the split.

**The write and the backup are two decisions, and conflating them survived one whole fix.** Gating the write on work done only closes the ZERO-repair case, and the field workdir is not that case: three of those four retypes drift by exactly one word and re-anchor, so the gate legitimately fires with `cutResult.changed` false — and an unconditional `.bak` refresh then destroys `splits: [0.6]` exactly as before, on the branch's own marquee success. So `refreshBackup` is `cutResult.changed`, never the gate (`writeOverrideDoc`). The asymmetry is the point: a cut re-anchoring rewrites absolute output-second VALUES all over the doc from a frame the pipeline just recomputed — unreadable in a diff, irreversible by hand, and precisely what the `.bak` exists for — while a caption-key migration differs from the copy on disk in caption KEYS alone, every one of them printed by name that run, deleting nothing. There is nothing in the old copy worth recovering, and the copy it would overwrite may be worth everything. The log line follows the same split, because "previous copy kept as .bak" over a run that took no backup is how someone finds out too late. Two writers sharing one single-generation backup file is the underlying hazard; this narrows who may spend it rather than fixing it, and `PUT /api/overrides` is deliberately not a third (see below).

**`MIGRATION_SEARCH_RADIUS = 8` is a judgement, and the two migration paths do not see the same drift.** The radius decides how much of a user's saved work the one-shot upgrade recovers, and the trade runs both ways: every extra word the outward scan considers is another chance a common word matches by coincidence, and the ambiguity rule turns a coincidence into a refusal — so widening it recovers more far-drifted edits and refuses more near ones. Eight is two or three seconds of speech, comfortably past the field case (a 0.6s trim moved every index by one). What makes the number affordable is that being past it is no longer a loss: a full scan on the failure path separates `out-of-range` (the word is right there, just further from the stored index than the search will trust) from `not-found` (it is genuinely gone), because those are the same silence to the code and opposite advice to the user. The divergence behind it is stated rather than closed: the editor migrates against the LAST run's `render-props.json` with a live memo that deliberately never applies `doc.cuts`, so its drift is zero and every legacy edit exact-hits, while `produce` migrates against lines built AFTER the new cut, where drift is the number of caption words that cut removed. A cut removing more than eight therefore shows every retype as applied in the preview and reports them `out-of-range` in the render. Closing that would mean re-implementing the EDL in the browser, which is the one thing the live memo exists to avoid — so `out-of-range` plus preservation is what keeps the gap a message instead of vanished work. Boundary tests sit at the radius and at radius+1, written against the exported constant so changing the value re-runs the question instead of failing arithmetic.

**Two write paths, two different answers about the `.bak`, and the reason they differ.** `PUT /api/overrides` still keeps no backup, deliberately: it is safe without one only because it ROUND-TRIPS — whatever the editor loaded it saves back, plus the change the user just made — and §137 briefly broke that property by stripping the unplaceable edits before `edits.load` (which also clears undo), so the first Save after opening a legacy project deleted them permanently with a dismissible banner and a `console.warn` as the whole record. The fix is in `migrateLoadedDoc`, which keeps them, not in the handler. Giving the handler produce's `.bak` was the other option on the table and is actively worse: the file is single-generation and SHARED with produce's write, so a routine ⌘S would spend the very backup the paragraph above exists to protect. The one loss channel left after that is the render: a completed run replaces the whole doc with one the editor did not write, `setRender(null)` discards the log that named anything produce dropped, and the reloaded doc is already clean — so `unresolved` and `dropped` are both empty and a missing retype would leave the transcript between one frame and the next with nothing on screen. `vanishedCaptionEdits` diffs the reloaded doc against the pre-render one BY CONTENT rather than by key, because the successful case is precisely a re-key (`"3"` → `"w2368"`) and a key diff would report every repair as a loss.
...
**What is still untested, stated rather than implied.** Nothing in the repo invokes `produce()`, and the editor's `renderflow.spec.ts` render is a fake child process, so the lines that wire this fix into the actual product — the `reconcileCaptionEdits` call, the `overrideDoc = captionWork.doc` that ADOPTS its result, and `captionKeysReanchored` in the override write gate — are executed by no test at all. Before the source-text guard below existed, deleting any one of them left the whole suite green. The adoption line is the nastiest of the three: dropping it typechecks (the result is still read for its lines and its log), the render silently loses the retypes, and because `reanchored` is still true the write gate fires anyway — overwriting the user's `.bak` with the un-migrated doc and printing that it re-anchored. Worse than never having called the migration, and invisible to both a compiler and a reviewer reading for a missing call. They are held for now by a source-text assertion in `caption-report.test.ts`, the same crude shape as the version-literal check (§113) and justified the same way: a behavioural test needs ffmpeg, a transcript, a workdir and a render, and what is being guarded is silent data loss. That is a stand-in for a `produce()`-over-`pnpm fixture` harness, not a substitute for one.

## 138. The project picker's rows crushed themselves — a flex item with `overflow: hidden` has a minimum height of zero

Reported from a screenshot, 2026-08-12, after weeks of the bug being live: the picker's browse rows were collapsed into each other, glyphs overlapping vertically, text unreadable; the recents above them were clipped at the descenders. The obvious read is a scroll container missing `min-height: 0` inside a flex parent, and it is wrong here — §83 already gave `browseSection` and `entryList` `flex: 1; min-height: 0; overflow-y: auto`, and they were correct. **The defect was on the CHILD side.** The rows are flex items in a column list, so they carry the default `flex-shrink: 1`; ordinarily a flex item's automatic minimum size (`min-height: auto`) would stop them shrinking below their content, but `overflow: hidden` — which the rows had, for the path ellipsis — sets that automatic minimum to **zero**. The rows therefore absorbed the whole overflow themselves. Measured in Chromium against a seeded picker: rows 18px and 22px against a natural 28px, and, decisively, `scrollHeight === clientHeight` on both lists. The container never overflowed, so `overflow-y: auto` never had anything to scroll. A scroll region and its rows are one mechanism, not two: the container can only scroll content that is allowed to be taller than it.

The fix is `flex-shrink: 0` plus an explicit `min-height` on the row, and it is the pair that matters — either alone leaves the other free to collapse. Both live in `entryButton` with the reason written beside them, because the failure mode of removing them is not a visual nudge but unreadable overlapping text, and nothing in a jsdom test can see it: jsdom has no layout, so the unit test can only assert the two declarations exist. The rendered proof is `e2e/picker.spec.ts`, which seeds nine recents and eighteen folders — past both containers' heights — and asserts a per-row bounding box of at least 24px and `scrollHeight > clientHeight`. Neither list overflows at the fixture's real sizes, which is exactly why a green suite carried this bug for weeks: the picker was only ever tested with a short list, and a short list cannot squash.

Two things surfaced while measuring and are fixed with it. The card declared `max-height: 82vh` with no `box-sizing: border-box` and no global reset, so its 26px padding and 1px border were added to the cap and it measured 868px against an 820px budget — invisible at a tall viewport, over the fold at a short one. And the row's colours moved out of the inline style object into `.ossclip-picker-row` in `index.css`: an inline `background: transparent` outranks every stylesheet rule short of `!important`, so hover and focus states written as CSS beside the existing scrollbar rules would have been dead code. The focus rule is `:focus` rather than `:focus-visible` deliberately — arrow-key navigation moves focus with a programmatic `focus()`, which Chromium does not reliably count as visible, and a keyboard user would be steering an invisible cursor.

The presentation change is the same finding seen from the user's side. Recents share a long identical prefix and differ only in their tails, so a single tail-ellipsised line truncated away the only part that told two entries apart. `splitRecentPath` cuts the path at its last three segments: the tail reads at full size, the shared head sits under it at 11px and is the part allowed to ellipsis. It is a pure function with the boundary cases (`/a/b/c` splitting to a head of `"/"` rather than `""`) tested without a DOM.

## 139. One key, two deletes: Delete/Backspace had to ask, because the editor has two unrelated things called "delete"

Asked for as a shortcut, 2026-08-12: "if a scene/cut is available, delete it, if only the original take is available, delete it. Or show a modal to select which one, as confirmation too." The request assumes the two deletes are alternatives on one axis and they are not. `hideScene` drops the GRAPHIC and keeps the window — the block goes ghost, the take underneath keeps playing for exactly as long, and the Inspector's Restore brings it back. `cutChunk` removes the WINDOW from the output on the next produce/Render, take or scene alike, and the preview deliberately keeps playing it (`live` in App.tsx never reads `doc.cuts`) so the only sign it happened is a struck-through block. Neither is a stronger version of the other and neither implies the other: a user who wanted the diagram off a shot they still need would lose four seconds of video, and a user cutting a fluff take would find the graphic gone and the fluff still there.

**The old binding was the "delete whichever is available" design, already shipped, and it is what makes the case against it.** Delete/Backspace called `hideScene` outright, refusing silently on a plain take (`cue.kind === "plain"`) and on an already-hidden one. So the same key did something, or nothing, depending on two pieces of state with no on-screen representation at the moment of the keypress — and the cut, the destructive one, was reachable only from an Inspector button most of the shortcut's users never opened. The confirm modal is not friction added to a working gesture; it is the first time the keyboard can reach both, and the first time the key's effect is stated before it happens.

**Confirming even when only one option applies is the point, not an oversight.** A dialog that appears for two options and silently acts for one is the original defect wearing a confirmation step — the user learns the key means "ask me", then one day it does not ask and something is gone. The single-option case therefore renders the identical radio row, checked, so what is about to happen is on screen either way. `deletePlanFor` returns `null` only when NOTHING is deletable (a hidden graphic on an already-cut window), which is the one honest case for the key to do nothing: a confirm dialog whose every option is unavailable is worse than silence.

**Split halves are where this could have quietly re-created §137.** A half's id is `${rootId}@${splitId}` with an opaque MINTED suffix, and the graphic delete dispatches against the half's OWN id — `hidden` is explicitly excluded from what `effectiveOverride` lets a half inherit, so there is no root entry to fall back to and none to accidentally hit. `deletePlanFor` uses `splitRootId` rather than touching the `@` itself, and it uses it for exactly one thing: the DISPLAY name. Putting `scene-0@600` in a dialog title invites the reading that cost §137 the field case — that the suffix is a time — so the title shows the root plus a `half` chip, and the graphic option's own copy says "this half only". The window offered to `cutChunk` is likewise the half's own post-split window, which only exists because the cue came from `splitCues`.

The decision is pure (`deletePlanFor`, `apps/editor/src/deleteScene.ts`) for the reason `ghostCues` and `renderCompleteReload` are: which options a given scene offers is the part worth testing, and it needs no DOM. The dialog is a `<form>` whose Delete is the submit button — that is what makes "Enter confirms the default" true from a focused radio as well as from the button, without reimplementing what the browser already does — and it traps Tab, which is not politeness: with focus loose behind a backdrop, Tab reaches the timeline's blocks and Space or Enter there edits the document the dialog is still asking permission about.

## 140. Per-phase timing: "3 hours" was unattributed, and the numbers say render, everywhere, by a mile

The complaint that forced this ("the process seems too slow", parked in next-directions §1) could not even be discussed: `produce_completed` carried one `duration_ms` for the whole run, so a slow run named no phase, and the four candidates — whisper, LLM planning, the Remotion render, ffmpeg concat/normalize — have completely different fixes if they dominate. The prior said render, because it is the only phase whose cost scales with OUTPUT frames rather than input seconds. Nothing said so with a number.

**The instrumentation.** `PhaseTimer` (apps/cli/src/phase-timing.ts, pure — clock injected) accumulates per-phase milliseconds: a phase is a SUM of calls (llm = repair + window selection + scenes; ffmpeg = concat + loudnorm), and a phase that never ran is ABSENT, never 0 — a cached transcript must not read as a 0.0s whisper run. Two consumers, split per the house layering: the run log prints one `▸ time:` line (real seconds, this is the user's own machine) with the unattributed remainder as `other`, so time this table doesn't cover accuses itself instead of hiding; and `produce_completed` gains one `<phase>_bucket` prop per phase that ran — bucketed at `<10s/10-60s/1-5m/5-15m/>15m`, finer than `durationBucket` because an 8s llm phase and a 40m render both being ">1m" would erase the exact comparison this exists to make, and never raw ms (§134: a per-phase duration is even closer to fingerprinting a take than the total). The bucket keys are pinned against `assertSafeProps` in phase-timing.test.ts because a spread into `telemetry.record` is invisible to telemetry.test.ts's source-text drift check.

**produce() finally has a test that runs it.** The §137 carry-out was right to weight this: both Criticals sat on a path guarded only by source-text regexes. produce-timing.test.ts executes produce() for real — folder input, real ffmpeg concat, real silence/level analysis — hermetic about the expensive edges (injected transcript, no LLM, no render), and asserts a measured ffmpeg phase, absent-not-zero for the phases that never ran, and the printed line.

**The measurement, 2026-08-12, same 7m05s synthetic 1080×1920 take on both machines, full render, no LLM:**

| | M3 Max laptop (14c, corporate) | M4 Mac mini (10c, clean) |
|---|---|---|
| total | 27m09s | 7m22s |
| whisper (small.en) | 44.1s | 18.6s |
| render | **23m19s (86%)** | **6m17s (85%)** |
| ffmpeg | 39.1s | 12.4s |
| other | 2m27s | 34.1s |

LLM planning, measured separately on the laptop (warm transcript, `--produce --no-render`, antigravity provider): **llm 51.6s** — repair plus a full scene plan for the 7-minute take, ~4% of the laptop's full-render wall clock, and cached in the workdir thereafter. The doc's "already cheap" survives contact, with one correction: it is a minute, not seconds, on a take this long.

Two findings, not one. (a) **Render dominates identically on both machines** — the hypothesis is confirmed, and whisper at small.en is noise (~1× realtime would still leave render at 85%). Hardware, more cores, or not rendering (the analyse-export direction, next-directions §2–3) are the only levers that matter; nothing else is worth an hour of tuning. (b) **The laptop is ~3.7× slower than a nominally weaker chip across every phase.** The mini's render even includes a one-time ~94MB Chrome Headless Shell download. Same code, same input, same phase shares — so the gap is the MACHINE's environment (corporate endpoint scanning is the standing suspect; VShieldScanner rides every process spawn, and a Remotion render is thousands of them), not ossclip. The per-phase line is what makes that conversation possible: "your laptop's render is 23m where a bare M4 does 6m" is an IT ticket, not a refactor.

**Idle A/B addendum, same day:** a controlled re-run on the 20s fixture with the laptop otherwise idle still renders 27.4s vs the mini's 12.0s — a chronic ~2.3× handicap with nothing else running (the 3.7× above includes load), on a machine with more cores (14 vs 10) and more RAM (36GB vs 16GB). RAM is exonerated by direction: the faster machine has less. What remains is the corporate endpoint stack.

## 141. `ossclip analyse` — the analyser without the renderer, and why the markers have no colours

§140's numbers made next-directions §3 concrete: the render is 85% of wall time, so the product that skips it is an order of magnitude faster by construction. `ossclip analyse take.mp4` (alias `analyze`) runs the same pipeline to the cut report — no LLM, no Remotion — and writes `take.fcpxml`: one labelled marker per planned cut, at frame-quantized SOURCE timestamps, named in the report.txt vocabulary (`silence −1.77s (conf 0.95)`). Markers, not applied cuts, on purpose: an editor wants to review "dead air here / blooper here" before acting, and 26 blind cuts is a much larger trust ask than 26 labels (next-directions §2). Design doc: docs/superpowers/specs/2026-08-12-analyse-fcpxml-export-design.md.

Decisions that will get asked about later:

- **No marker colours.** The doc's open question — do reason-mapped colours survive a Resolve round trip — resolves to NO reliable path: stock FCPXML `<marker>` has no colour attribute at all (colour is an application-side property in both Resolve and FCP), and the to-do/chapter marker variants change semantics, not colour. The reason travels in the marker NAME, which every importer displays. Revisit only if a real editor asks.
...
**Workflow addendum, same evening, from the first real editor:** sequence markers alone are not usable in anger. Her loop is remove-as-you-go — and sequence markers sit at fixed timecode, so after the first ripple delete every downstream marker points at the wrong moment; she went back to manual removal. The fix is marker ANCHORING, not marker presence: markers now also live on the clipitem (clip markers are anchored to media time), so they ride through razor cuts and ripple deletes and stay on the words they describe. The sequence copies remain as the read-only overview. Premiere sequence properties from the same session also forced the sequence-level `samplecharacteristics` block — without it Premiere invents a 720×480 DV sequence and the take renders cropped ("the orientation is off").

**Round 2, next morning's recorded feedback (2026-08-12 evening):** three exporters, one wrong idea, verified frame-by-frame against her screen recording. (a) **Point markers were the wrong shape.** All complaints — "the video starts over here", "a mark here AND here so I can see this part is twice", even "the starting point is incorrect" — reduce to: we marked where each cut BEGINS and threw away where it ENDS. The 1-frame choice was a deliberate "don't overstate the suggestion"; the field reversed it — the span IS the suggestion. All three formats now carry the full srcIn→srcOut range (FCPXML `duration`, xmeml `<in>/<out>`, EDL record span + `|D:`). (b) **Detected-but-kept pauses were invisible.** Real pauses sat in her waveform with no marker: pauses the analysis found but the cut rules kept. `analysis.cuttable` minus remove-overlapped regions, floored at 0.25s (`MIN_KEPT_PAUSE_SEC` — a 30-minute take has hundreds of sub-quarter-second breaths, and marker spam buries the markers she asked for), exports as its own marker class — Lavender in Resolve, `pause 0.42s (kept)` everywhere. (c) **The xmeml had no audio track**, so the sequence imported silent; a linked `ci-2` audio clipitem fixes it, gated on `probe.hasAudio` with no dangling `linkclipref` when absent. The 0.1.21 bump was reverted mid-flight for this — the runbook's "nothing lands between bump and publish" surviving its first live test.

## 143. The agy hang would not reproduce, so the fix is an exit, not a cure

§132's incident closed with a corrected error message; this section is what happened when the underlying hang was chased. The failing call was ~63k input tokens AND carried the full beat-sheet JSON schema, and the two candidate causes — scale and schema — had never been separated. Three bounded probes (agy 1.1.18, `--print-timeout 3m`, same machine, 2026-08-22) separated them:

| probe | prompt | schema | result |
| --- | --- | --- | --- |
| A | 23.6KB (33,428 in tokens) | real beat-sheet schema | validation ERROR in 46s |
| B | 173KB (95,030 in tokens) | toy 3-field | **SUCCESS in 17.6s** |
| C | 173KB (91,140 in tokens) | real beat-sheet schema | validation ERROR in 23s |

**Both suspects are innocent.** The real schema, dumped from `z.toJSONSchema(BeatSheetSchema)`, is 1,299 bytes of clean JSON Schema — the `cappedText` preprocess leaves no artifacts. A 95k-token call — half again the size of the one that hung — succeeded in 17.6 seconds. And the combination, probe C, failed *fast*, not slow. The hang did not reproduce under any of the three, hours after two consecutive 10-minute expiries on the real call: it is intermittent and lives on agy's service side, which is exactly the kind of failure you route around rather than fix.

Two smaller corrections rode along. First, §132's "server-enforced structured output" oversold the enforcement: probes A and C came back with `sceneKind: "hook"` and 147-char `purpose` fields — the model decodes unconstrained, the service *validates* afterwards and returns `status: "ERROR"` with a JSON-pointer issue list. Enforcement is a bouncer, not a rail, which is one more reason the provider's own `schema.parse()` stays. Second, the handoff's "extracting the schema via `npx tsx` hit import errors" mystery: the workspace root pins zod 3.25 (no `toJSONSchema`), while `packages/core` imports `zod/v4` — the dump works from anywhere once the import says `zod/v4`.

**The fix, given an intermittent cause, is a fallback, not a knob.** Three decisions, all landed 2026-08-22:

- **A timed-out attempt is never retried against agy** (the retry loop breaks on the first `timeout` classification). Measured twice on the real call: the second attempt only doubled the wall clock to 20 minutes. The terminal error is now an `AgyError` carrying `failureClass` as data, because the next decision needs to branch on it without re-parsing prose.
- **On `timeout` — and only that class — the call falls back once** to the provider the auto-detection order would have picked next (claude-cli if the CLI is installed, gemini on `GEMINI_API_KEY`), via a `FallbackProvider` decorator in the same shape as `TieredProvider`. Auth and model failures keep failing fast; falling back on them would hide a config error behind a different bill. The fallback is announced with a `⚠` line naming both providers — silent substitution would be worse than the hang, because the user must know which model planned their video.
- **A fallback-planned result caches under the answering provider's key**, not the requested one — the cache key records what actually planned the video. Deliberate consequence: a repeat `--llm antigravity` run re-attempts agy (10 minutes now, not 20) before falling back again, and a later `--llm claude-cli` run on the same workdir hits the fallback-written cache. Per-record usage attribution was already truthful (`LlmUsage.provider`); the report header now names both providers when a run spans them.
...
## 155. A positional id is a name, not an identity

Scene overrides were keyed by the id the planner mints positionally — `scene-${i}` (scene-props.ts) — and `applyOverrides` joined the user's edit layer onto cues purely on that id. Both halves are individually reasonable; together they made the id carry a promise it cannot keep. A re-plan renumbers freely, so `scene-4` in the new plan can be a DIFFERENT moment than the `scene-4` the user edited — and the edit then applied to it silently. A shrunk plan at least tripped the orphan warning (§150's field case, `edit for scene-11 dropped`); a same-size renumber tripped nothing, which is worse: the framing tuned for one graphic lands on another, and every signal says the edit applied fine.

The design was gated on an empirical question — is anything about a moment actually stable across re-plans? — and the answer was unambiguous. Two real planner runs over the same Handy transcript: 11/11 and 8/8 moments matched their counterpart by word-anchor overlap, every pair at 100%, while 8/11 and 2/10 ids shifted. One observed case makes the misapply concrete: `scene-4` was a TerminalMock over words 85..116 in one plan and a FlowDiagram over words 47..57 in the next. The anchors are the identity; the ids are just the order the planner happened to emit them in.

So the anchor became the join key, in three moves. Cues now carry their plan anchor through assembly. The editor stamps each scene override with the anchor of the cue the user was looking at, at save time, from the cues in its own memory — never from `render-props.json` on disk, because a mid-session re-render can make disk describe a newer plan than the screen, and stamping from it would record the wrong identity, which is the misapply wearing yet another face. And produce runs `remapSceneOverrides` immediately after `assembleScenes`, before ANYTHING consumes the keys. That placement was a fix-round correction, and it is the lesson worth keeping: the first landing put the remap post-fill, which left the first `applyOverrides` pass, the hidden-drop and the pinned-timing reclamp all joining STALE ids — a `hidden` on a renumbered scene would hide the impostor before the remap ever ran. A remap placed after any consumer of the keys is the original bug in a smaller mask; the only correct position is before all of them.

The genuinely-gone case gets a rule of its own. An entry whose words left the plan while its id now names a different moment is PARKED under `key#orphaned` — `#` appears in no cue id, so the entry is unjoinable, with its data and anchor intact — and rescued the day a later plan has its words again. Never left under a key the impostor answers to. After the remap, "dropped" in the console finally means what it says: both identities missed.

This is the scene-side sibling of §137, deliberately down to the migration posture: captions moved from positional indices to source-time keys when a user cut broke the positions; scenes move from positional ids to word anchors when a re-plan breaks the numbering. Same failure (a positional key mistaken for an identity), same fix (key on the property the disruption cannot move), same stance on old data — a doc written before the anchor field keeps id-only behaviour, gaining no protection and losing nothing it had.

## 156. A playhead captured on one clock and spent on another is the same wrong frame every time

Reported on the published 0.1.29: regenerating the cover from the editor "did nothing" — the picture never changed, whatever was set. It changed every time. "Use current playhead" captured seconds on the FINISHED video's clock; "Original take" (`--from source`) spent that number on the SOURCE clock; nothing converted. So the panel extracted a real, fresh frame from the wrong moment — and because it was the same wrong moment on every attempt, it read as "nothing happened", which is a far better disguise than an error. The sharpest part is that the panel's own `playheadSec` contract had already written the gap down — "the server re-maps nothing for it" — in the very comment describing the prop. A known gap is not a closed gap; a contract that admits a defect is a TODO wearing documentation's clothes, and it sat there through a release.

One sentence on the theory that preceded it, because the handoff already paid for the lesson: a stale-image-cache story fit the symptom perfectly and was disproven only by the server's own log line naming "45.0s of the source video" — when a UI "doesn't update", make the server say what it actually did before theorising about the client.

The design decision is a semantics decision before it is a fix: the seconds field means seconds on the SELECTED video's clock, always. That is what `ossclip cover --at` already means, so the server keeps re-mapping nothing; and it is the only denomination that keeps cut-out source moments reachable — a field always in output time could never *name* an instant the cut removed, and reaching those instants is half of why `--from source` exists. Conversion therefore happens only at the two gestures that genuinely cross clocks: the playhead button (output → source, `TimeMap.toSource`) and the final ↔ source toggle (both directions — with the source → final direction clamping to the nearest kept moment, under a spoken note, when the instant was cut rather than passing the snap off as a conversion). The ruler for both is the finished mp4's RESOLVED cutlist, which now rides `/api/cover` — the proposal `/api/cleanup` serves is the wrong ruler the moment a user's vetoes have changed it, and measuring the finished video with the proposal is this bug in a second costume.

Two smaller fixes are what made the bug diagnosable at all, and they earn their place here for that reason:

- **The seconds field no longer resets after Apply.** The blank-on-success reset meant you could never see what had just been used, let alone iterate from it — a large part of why the clock mismatch took a log line to characterise instead of a glance.
