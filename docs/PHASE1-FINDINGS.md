...
## 79. `ossclip backfill` — recovering provenance that predates §78

§78 stops the erasure going forward, but the workdirs already flattened stay flat. `ossclip backfill <paths…>` reconstructs what is still recoverable: it takes a workdir or a root full of them (`~/Videos/.ossclip`), reads the provider out of each one's own recorded `command.json` argv, and writes a single run entry marked `backfilled: true` with a note saying where the provider came from. What it will not do is invent: token counts and model names for those runs are genuinely gone, so they stay empty; a log that already has a real history is skipped outright; a `producer` stamp a real run wrote is never overwritten; and surviving records beat the argv, since a recorded provider is a fact and a flag only ever a hint. `--dry-run` previews, originals are copied to `.pre-backfill` unless `--no-backup`, and re-running is a no-op. Verified against the real `~/Downloads/.ossclip`: the already-recovered workdirs report "already has a run history", and the two with no `command.json` say exactly that instead of guessing.
