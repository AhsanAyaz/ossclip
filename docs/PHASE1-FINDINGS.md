...
**Keep-last, and its stricter survivor bar.** Kept = the last live COMPLETE instance in a chain — one that ends at real sentence punctuation, not a sub-split boundary — whose own `silenceFrac` clears `RESTART_SPLIT_MIN_SIL` (0.35). When NO complete instance in the chain clears that bar, the group goes report-only instead: `kept` is `null`, nothing cuts, and every real instance in the chain is listed with its own `silenceFrac` so the report can say WHY nothing was decided — falling back to "last complete regardless" would silently elect the gappiest instance available, exactly the failure the bar exists to catch, just moved one level up. When a survivor does clear the bar, every other real instance in the chain cuts: earlier completes, every partial, a trailing abandoned restart with nothing after it. Keep-last is a convention, not a proof — the true best take isn't always the last one recorded — and is documented as a known limit, the same posture §122 already takes toward its own marker-based removal.
